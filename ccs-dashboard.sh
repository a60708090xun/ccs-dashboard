...
CCS_DISPATCH_WAIT_POLL_INTERVAL=${CCS_DISPATCH_WAIT_POLL_INTERVAL:-2}
...
_ccs_dispatch_jsonl_append() {
  ...
  local exit_code="$9" summary="${10}" created_at="${11}"
  ...
  [ -z "$created_at" ] && created_at="$(date -Iseconds)"
...
_ccs_dispatch_context() {
  ...
  # Git info (if git repo — .git can be dir or file in worktrees)
...
# ── Dispatch: Lazy Status Sync (for ccs-jobs) ──
...
ccs-dispatch() {
  local sync=false context=false notify=false timeout_secs="$CCS_DISPATCH_TIMEOUT"
  local project="" task=""
...
Dispatch a task to a new Claude Code session via claude -p.
  ...
  --notify          Print notification to stderr when async job completes
...
HELP
  ...
  if $context && ! git -C "$project" rev-parse --git-dir &>/dev/null; then
  ...
  ' "$dispatch_dir/jobs.jsonl" 2>/dev/null | tr -d '[:space:]')
  ...
  if $sync; then
    ...
    local ready_file="$dispatch_dir/pids/${job_id}.ready"
    (
      # Wait for main shell to signal readiness (PID file + JSONL written)
      local ready_wait=0
      while [ ! -f "$ready_file" ] && [ "$ready_wait" -lt 30 ]; do
        sleep 0.1; ready_wait=$(( ready_wait + 1 ))
      done
      rm -f "$ready_file"
      if [ "$ready_wait" -ge 30 ]; then
        echo "Error: dispatch ready timeout for $job_id" >&2
        exit 1
      fi
      cd "$project" && timeout "$timeout_secs" claude -p "$prompt" \
        > "$dispatch_dir/results/${job_id}.out" \
    ...
    # Write PID file and JSONL, then signal subshell to start
    ...
    # Start background notifier if requested
    if $notify; then
      (
        local elapsed=0
        local max_wait=$(( timeout_secs + 60 ))
        while kill -0 "$bg_pid" 2>/dev/null && [ "$elapsed" -lt "$max_wait" ]; do
          sleep "$CCS_DISPATCH_WAIT_POLL_INTERVAL"
          elapsed=$(( elapsed + CCS_DISPATCH_WAIT_POLL_INTERVAL ))
        done
        if [ "$elapsed" -ge "$max_wait" ]; then
          printf '\n⚠️  Notify watcher for %s timed out after %ds\n' "$job_id" "$max_wait" >&2
        else
          _ccs_dispatch_sync_status "$dispatch_dir"
          local exit_code
          exit_code=$(jq -sc --arg id "$job_id" '
            [.[] | select(.job_id == $id)] | last | .exit_code // "?"
          ' "$dispatch_dir/jobs.jsonl" 2>/dev/null | tr -d '[:space:]')
          local icon="✅"; [ "$exit_code" != "0" ] && icon="❌"
          printf '\n%s Job %s finished (exit %s)\n' "$icon" "$job_id" "$exit_code" >&2
        fi
      ) &
      disown
    fi
