...
HELP
  ...
  for f in "${open_files[@]}"; do
    ...
    if [ -n "${crash_full[$full_sid]+x}" ] && [ "$ago" -lt $_CCS_CRASH_EXPIRY_MINS ]; then
      crashed_files+=("$f")
  ...
  if [ ${#stale_files[@]} -eq 0 ]; then
    ...
    printf "  \033[90m%d open session(s) inactive (stale or crash-expired)\033[0m → \033[33mccs-sessions 168\033[0m to inspect\n" "${#stale_files[@]}"
  fi
}
...
_ccs_status_md() {
  ...
  if [ ${#stale_files[@]} -eq 0 ]; then
    ...
    echo "> **${#stale_files[@]}** open session(s) inactive (stale or crash-expired) → \`ccs-sessions 168\` to inspect"
  fi
}
