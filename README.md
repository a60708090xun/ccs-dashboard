...
| `ccs-dispatch` | 派工到新的 Claude Code session（同步/非同步） |
| `ccs-jobs` | 查看 dispatch job 歷史與結果 |
...
派工到新的 Claude Code session，以 `claude -p` 非互動模式執行。預設非同步（背景執行），加 `--sync` 可同步等待結果。

```bash
ccs-dispatch --project /path/to/project "fix lint warnings"           # 非同步
ccs-dispatch --sync --project /path/to/project "run tests"            # 同步等結果
ccs-dispatch --context --project /path/to/project "refactor utils"    # 注入 git 狀態 + todos
ccs-dispatch --timeout 300 --project /path/to/project "quick task"    # 自訂超時
ccs-dispatch --notify --project /path/to/project "long running task"  # 完成後通知
```

| 參數 | 預設 | 說明 |
|------|------|------|
| `--project <dir>` | **必填** | 目標專案目錄 |
| `--sync` | 否 | 同步等待結果 |
| `--context` | 否 | 注入目標專案的 git 狀態 + active session 待辦 |
| `--timeout <secs>` | 600 | 任務超時秒數 |
| `--notify` | 否 | 非同步模式下，完成後在 stderr 印出通知（有 max timeout 防護） |

可配置環境變數（在 `.bashrc` 中設定即可覆蓋預設值）：

```bash
CCS_DISPATCH_TIMEOUT=600                 # 任務超時秒數
CCS_DISPATCH_JOBS_LIMIT=20               # ccs-jobs 預設列出筆數
CCS_DISPATCH_TASK_DISPLAY_LEN=60         # 任務摘要顯示字數
CCS_DISPATCH_RESULT_TTL_DAYS=7           # result 檔保留天數
CCS_DISPATCH_MAX_CONCURRENT_WARN=3       # 並行 job 警告門檻
CCS_DISPATCH_WAIT_POLL_INTERVAL=2        # --notify 輪詢間隔秒數
CCS_DISPATCH_SUMMARY_LINES=30            # 結果摘要擷取行數
CCS_DISPATCH_SUMMARY_MAX_CHARS=200       # 結果摘要最大字數
```

### ccs-jobs

查看 dispatch job 歷史與結果。

```bash
ccs-jobs                  # 列出最近 20 筆 jobs
ccs-jobs --all            # 列出所有 jobs
ccs-jobs d-20260319-1430  # 查看指定 job 的完整結果
```

資料存放：`${XDG_DATA_HOME:-$HOME/.local/share}/ccs-dashboard/dispatch/`
